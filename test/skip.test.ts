...
function getIter(skip: number) {
  return {
    iter: LazyIter.from(src).skip(skip),
    values: src.slice(skip)
  };
}
...
describe('SkipIter', () => {
  ...
  describe('nth', () => {
    it('should return the nth item in the iterator', () => {
      ...
      expect(iter.nth(2).value).toEqual(5);
  ...
  describe('zip', () => {
    ...
    it('should stop iterating as soon as one iterable is done (left first)', () => {
      const { iter, values } = getIter(2);
      const other = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l'];
      util.expectToIterateOverValues(iter.zip(other), util.getPairs(values, other));
    });
